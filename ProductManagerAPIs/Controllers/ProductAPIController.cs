...
using ProductManagerAPIs.Models;
...
namespace ProductManagerAPIs.Controllers
{
    //[RoutePrefix("product")]
    public class ProductAPIController : ApiController
    {
        ...
        private ResponseModel responseObj;

        #region response formatter
        private System.Web.Http.Results.NegotiatedContentResult<T> ReturnFormatedResponseGen<T>(T obj, HttpStatusCode httpStatusCode)
        {
            return Content(httpStatusCode, obj);
        }

        private System.Web.Http.Results.NegotiatedContentResult<T> ReturnNoContent<T>(T obj)
        {
            return Content(HttpStatusCode.NoContent, obj);
        }

        private System.Web.Http.Results.NegotiatedContentResult<T> ReturnOK<T>(T obj)
        {
            return Content(HttpStatusCode.OK, obj);
        }

        private System.Web.Http.Results.NegotiatedContentResult<T> ReturnCreated<T>(T obj)
        {
            return Content(HttpStatusCode.Created, obj);
        }

        private System.Web.Http.Results.NegotiatedContentResult<T> ReturnBadRequest<T>(T obj)
        {
            return Content(HttpStatusCode.BadRequest, obj);
        }

        private System.Web.Http.Results.NegotiatedContentResult<T> ReturnUnauthorized<T>(T obj)
        {
            return Content(HttpStatusCode.Unauthorized, obj);
        }

        private System.Web.Http.Results.NegotiatedContentResult<T> ReturnInternalServerError<T>(T obj)
        {
            return Content(HttpStatusCode.InternalServerError, obj);
        }

        private System.Web.Http.Results.NegotiatedContentResult<T> ReturnInternalServerErrorAsBadRequest<T>(T obj)
        {
            return Content(HttpStatusCode.BadRequest, obj);
        }
        #endregion
        ...
        public HttpResponseMessage GetCategories()
        {
            ...
            try
            {
                ...
                if (modelList == null || modelList.Count == 0)
                {
                    ...
                    json = ApiVariables.ResponseJson(1, responseMessage, null);
        ...
        public IHttpActionResult GetCategories(Int32 id)
        { 
            try
            {
                var modelList = Repository.GetCategoryList();
                ...
                if (modelList == null || modelList.Count == 0)
                { 
                    responseMessage = OutputModel.GetNotFoundError("categories");
                    responseObj = ApiVariables.ResponseObj(1, responseMessage);
                    return ReturnNoContent(responseObj); 
                }

                var model = modelList.Where(c => c.id == id).FirstOrDefault();
                //Check whether list is empty
                if (model == null || model.id <= 0)
                { 
                    responseMessage = OutputModel.GetNotFoundError("category");
                    responseObj = ApiVariables.ResponseObj(1, responseMessage);
                    return ReturnNoContent(responseObj);
                }

                var obj = new { Result = model };

                responseObj = ApiVariables.ResponseObj(0, "Success", obj);
                return ReturnOK(responseObj);
            }
            catch (Exception ex)
            { 
                responseMessage = OutputModel.GetException(ex.Message);
                responseObj = ApiVariables.ResponseObj(1, responseMessage);
                return ReturnInternalServerErrorAsBadRequest(responseObj); 
            } 
        }
        ...
        public HttpResponseMessage GetTypes()
        {
            ...
            try
            {
                ...
                if (modelList == null || modelList.Count == 0)
                {
                    ...
                    responseMessage = string.Format("No data found");
        ...
        public HttpResponseMessage PostType([FromBody]JToken postData)
        {
            ...
            try
            {
                ...
                if (requestObj.systemTypeId<=0)
                {
                    responseMessage = OutputModel.GetEmpty("System Type");
                    json = ApiVariables.ResponseJson(1, responseMessage);
                ...
                { 
                    systemTypeId=requestObj.systemTypeId,
                    name = requestObj.name,
        ...
        public HttpResponseMessage GetProducts(Int32 categoryId)
        {
            ...
            try
            {
                ...
                if (modelList == null || modelList.Count == 0)
                {
                    ...
                    responseMessage = string.Format("No data found for categoryId: {0} ", categoryId);
        ...
        public HttpResponseMessage GetProductSubAttributes(Int32 attributeId)
        {
            ...
            try
            {
                ...
                if (modelList == null || modelList.Count == 0)
                {
                    ...
                    responseMessage = string.Format("No data found for AttributeId: " + attributeId);
    ...
    public class ApiVariables
    {
        ...
        public static ResponseModel ResponseObj(byte rCode, string message, dynamic responseObj = null)
        {
            var sModel = new ResponseModel
            {
                rCode = rCode,
